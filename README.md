...
- `code`: contains scripts for the Naive optimization model and the Storage-Enhanced optimization model, scenario generation, statistical post-processing, and visualization.
  ...
  - Optimization: `naive_optimization_models.py` (Naive model, also available as a 1000-scenario robustness variant in `naive_optimization_models_1000.py`); `parallel_battery.py` / `parallel_battery_model_cap.py` (Storage-Enhanced model, parallel execution across countries, tolerances, forecast dates, and installed-capacity scalings); `parallel_battery_cap_sensitivity.py` (battery-sizing sensitivity)
