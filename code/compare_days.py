#!/usr/bin/env python3
"""
Cross-day comparison of mean synergy ratios across all four rolling windows.

Outputs:
  results/comparison_heatmap_days.png   — annotated country × day heatmap
  results/comparison_lineplot_days.png  — line plot (countries in all 4 days)
  results/comparison_all_days.csv       — merged table
"""
...
frames = []
for day in DAYS:
    df = pd.read_csv(f"{RESULTS}/battery_CI_day{day}_tol005_384.csv")
    df["day"] = day
    frames.append(df)
...
# Save merged table
df_all.to_csv(f"{RESULTS}/comparison_all_days.csv", index=False)
print(f"Saved: {RESULTS}/comparison_all_days.csv")
...
# Build annotation matrix: value if present, "—" if missing
...
ax.set_title(
    ...
    "(tol=0.05, 384 MW installed capacity)",
    fontsize=13, pad=12
)
...
plt.tight_layout()
hm_path = f"{RESULTS}/comparison_heatmap_days.png"
...
country_name_map = {
    ...
    "FI": "Finland",    "FR": "France",    "IE": "Ireland",
...
regions = {
    ...
    "Continental":       ["DE", "PL", "AT"],
    "Mediterranean":     ["ES", "PT", "EL", "RO"],
    "Nordic & Baltic":   ["NO", "SE", "FI", "DK", "LT"],
}
...
ax.set_title(
    ...
    "(countries present in all four days, tol=0.05, 384 MW)",
    fontsize=13
)
ax.set_xlim(-0.15, len(DAYS) - 1 + 1.5)
...
plt.tight_layout()
lp_path = f"{RESULTS}/comparison_lineplot_days.png"
...
print("\n── Summary: range of mean ratios across days ──")
