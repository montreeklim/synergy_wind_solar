#!/usr/bin/env python3
"""
General analysis pipeline for battery results — 1x installed capacity (100 MW).

Usage:
    python analyze_battery_results_100.py --day 271 --tol 0.05
    python analyze_battery_results_100.py --day 362 --tol 0.01

Reads  : ../battery_results/100_installed_capacity/battery_{COUNTRY}_day_{DAY}_tol_{TOL}_100_installed_capacity.csv
Outputs: ../results/battery_CI_day{DAY}_tol{TAG}_100.csv
         ../results/boxplot_day{DAY}_tol{TAG}_100.png
         ../results/synergy_heatmap_day{DAY}_tol{TAG}_100.png
         ../results/decision_tree_day{DAY}_tol{TAG}_100.png
"""
...
# Fix GDAL data path for pyogrio/geopandas on Windows
import sys
...
# e.g. 0.05 -> "005", 0.01 -> "001", 0.10 -> "010"
tol_tag = f"{TOL:.2f}".replace("0.", "").replace(".", "")

ROOT        = ".."
RESULTS_DIR = "../results"
PATTERN     = os.path.join(ROOT, "battery_results", "100_installed_capacity",
                           f"battery_*_day_{DAY}_tol_{TOL}_100_installed_capacity.csv")
TAG         = f"day{DAY}_tol{tol_tag}_100"

print(f"\n=== Day {DAY} | tol={TOL} | 100 MW (1x) installed capacity ===\n")

# ── 1. Compile per-country CSVs ───────────────────────────────────────────────
...
# ── 2. 95% CIs (t-distribution) ───────────────────────────────────────────────

def ci_bounds(data):
    n = len(data)
    if n < 2 or np.std(data) == 0:
...
for country, grp in df_all.groupby("country"):
    ...
    ci_rows.append({
        ...
        "ci_lower":   round(float(lo), 4) if not np.isnan(lo) else np.nan,
        "ci_upper":   round(float(hi), 4) if not np.isnan(hi) else np.nan,
        "ci_string":  f"({lo:.4f}, {hi:.4f})" if not np.isnan(lo) else "N/A",
    })
...
ci_path = os.path.join(RESULTS_DIR, f"battery_CI_{TAG}.csv")
...
ax.set_title(f"Synergy Ratio by Region\n(day_{DAY}, tol={TOL}, 1x installed capacity)", fontsize=13)
plt.xticks(rotation=30, ha="right", fontsize=13)
plt.yticks(fontsize=13)
plt.tight_layout()
bp_path = os.path.join(RESULTS_DIR, f"boxplot_{TAG}.png")
plt.savefig(bp_path, dpi=300)
plt.close()
print(f"Saved: {bp_path}")

# ── 4. Geographic synergy heatmap ─────────────────────────────────────────────
...
df_map = df_ci[["country", "mean_ratio"]].rename(columns={"country": "ISO2"}).copy()
...
df_has  = df_geo[df_geo["mean_ratio"].notna()].copy()
df_miss = df_geo[df_geo["mean_ratio"].isna()].copy()

missing_countries = sorted(df_miss.dissolve(by="ISO2").index.tolist())

fig, ax = plt.subplots(figsize=(12, 8), subplot_kw={"projection": PlateCarree()})

vmin = 1.0
vmax = df_has["mean_ratio"].max()
norm = Normalize(vmin=vmin, vmax=vmax)
cmap = "YlGn"

df_has.plot(column="mean_ratio", cmap=cmap, norm=norm,
...
if missing_countries:
    label = f"No data: {', '.join(missing_countries)}"
    patch = mpatches.Patch(color="#CCCCCC", label=label)
    ax.legend(handles=[patch], loc="lower left", fontsize=9)

ax.set_title(
    f"Mean Synergy Ratio by Country\n(day_{DAY}, tol={TOL}, 1x installed capacity)",
    fontsize=15, pad=14,
)
hm_path = os.path.join(RESULTS_DIR, f"synergy_heatmap_{TAG}.png")
plt.savefig(hm_path, dpi=300, bbox_inches="tight")
plt.close()
print(f"Saved: {hm_path}")

# ── 5. ML features (fixed — from EMHIRES) ────────────────────────────────────
...
df_ml = df_features.join(
    df_ci[["country", "mean_ratio"]].set_index("country"), how="inner"
)
missing_ml = sorted(set(features_by_country) - set(df_ml.index))
print(f"\nML analysis: {len(df_ml)} countries | excluded (no data): {missing_ml}")
...
plot_tree(dt, feature_names=feat_cols, filled=True, rounded=True,
          precision=3, fontsize=9)
plt.title(f"Decision Tree — Synergy Ratio (day_{DAY}, tol={TOL}, 1x installed capacity)", fontsize=13)
plt.tight_layout()
dt_path = os.path.join(RESULTS_DIR, f"decision_tree_{TAG}.png")
