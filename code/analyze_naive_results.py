#!/usr/bin/env python3
"""
Analysis pipeline for naive (no-battery) rolling-window results.

Usage:
    python analyze_naive_results.py --day 271 --tol 0.05
    python analyze_naive_results.py --day 362 --tol 0.01

Reads  : ../naive_results/naive_results_*.csv  (28 per-country files)
Outputs: ../results/naive_CI_day{DAY}_tol{TAG}.csv
         ../results/naive_boxplot_day{DAY}_tol{TAG}.png
         ../results/naive_heatmap_day{DAY}_tol{TAG}.png
         ../results/naive_decision_tree_day{DAY}_tol{TAG}.png
"""
...
print(f"\n=== Naive model | Day {DAY} | tol={TOL} ===\n")

# ── 1. Load and merge all per-country CSVs ────────────────────────────────────

files = sorted(glob.glob("../naive_results/naive_results_*.csv"))
...
ci_path = os.path.join(RESULTS_DIR, f"naive_CI_{TAG}.csv")
...
cap = df_plot["ratio"].quantile(0.99)
df_plot["ratio_capped"] = df_plot["ratio"].clip(upper=cap)
...
sns.boxplot(data=df_plot, x="Region", y="ratio_capped",
...
ax.set_title(f"Naive Model — Synergy Ratio by Region\n(day_{DAY}, tol={TOL})", fontsize=13)
plt.xticks(rotation=30, ha="right", fontsize=13)
plt.yticks(fontsize=13)
plt.tight_layout()
bp_path = os.path.join(RESULTS_DIR, f"naive_boxplot_{TAG}.png")
...
vmin = 1.0
vmax = df_has["mean_ratio"].quantile(0.95)   # cap colorscale at 95th pct to handle outliers
...
ax.set_title(
    f"Naive Model — Mean Synergy Ratio by Country\n(day_{DAY}, tol={TOL})",
    fontsize=15, pad=14,
)
hm_path = os.path.join(RESULTS_DIR, f"naive_heatmap_{TAG}.png")
...
df_ml = df_features.join(df_ci[["country", "mean_ratio"]].set_index("country"), how="inner")
...
plt.title(f"Decision Tree — Naive SR (day_{DAY}, tol={TOL})", fontsize=13)
plt.tight_layout()
dt_path = os.path.join(RESULTS_DIR, f"naive_decision_tree_{TAG}.png")
