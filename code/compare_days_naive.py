#!/usr/bin/env python3
"""
Cross-day comparison of naive model mean synergy ratios.

Usage:
    python compare_days_naive.py --tol 0.05
    python compare_days_naive.py --tol 0.01

Outputs:
  results/naive_comparison_all_days_tol{TAG}.csv
  results/naive_comparison_heatmap_tol{TAG}.png
  results/naive_comparison_lineplot_tol{TAG}.png
"""
...
frames = []
for day in DAYS:
    df = pd.read_csv(f"{RESULTS}/naive_CI_day{day}_tol{tol_tag}.csv")
    df["day"] = day
    frames.append(df)
...
out_csv = f"{RESULTS}/naive_comparison_all_days_tol{tol_tag}.csv"
...
# Cap colorscale so extreme values don't wash out the map
...
ax.set_title(
    f"Naive Model — Mean Synergy Ratio by Country and Rolling Window\n(tol={TOL})",
    fontsize=13, pad=12
)
...
plt.tight_layout()
hm_path = f"{RESULTS}/naive_comparison_heatmap_tol{tol_tag}.png"
...
ax.set_xlabel("Rolling Window", fontsize=12)
ax.set_title(
    f"Naive Model — Synergy Ratio across Rolling Windows\n(tol={TOL})",
    fontsize=13
)
...
plt.tight_layout()
lp_path = f"{RESULTS}/naive_comparison_lineplot_tol{tol_tag}.png"
...
print(f"\n-- Summary: range of mean ratios across days (naive, tol={TOL}) --")
