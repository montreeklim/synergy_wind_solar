...
for f in glob.glob('../battery_results/100_installed_capacity/*.csv'):
    ...
    if not m:
        continue
    country, day, tol = m.group(1), int(m.group(2)), float(m.group(3))
    tmp = pd.read_csv(f)
    tmp['country'] = country
    tmp['forecast_day'] = day
    tmp['tol'] = tol
    dfs.append(tmp)
...
TOL_LABEL_MAP = {0.01: 'ε = 0.01', 0.05: 'ε = 0.05', 0.1: 'ε = 0.10'}
...
plt.tight_layout()
out = '../results/battery_regional_boxplot_combined.png'
